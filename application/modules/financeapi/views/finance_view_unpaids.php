	...
	<tbody>
		<?php 
			...
			if($keywords && $this->session->userdata('user_type') === 'Org Admin') {
				// January
			$jan_date = $year.'-01-';
			$jan_payment = get_month_wise_fees($jan_date, $item['org_id'],'UNPAID',$keywords);
			
			// February
			$feb_date = $year.'-02-';
			$feb_payment = get_month_wise_fees($feb_date, $item['org_id'],'UNPAID',$keywords);
			
			// March
			$mar_date = $year.'-03-';
			$mar_payment = get_month_wise_fees($mar_date, $item['org_id'],'UNPAID',$keywords);
			
			// April
			$apr_date = $year.'-04-';
			$apr_payment = get_month_wise_fees($apr_date, $item['org_id'],'UNPAID',$keywords);
			
			//May
			$may_date = $year.'-05-';
			$may_payment = get_month_wise_fees($may_date, $item['org_id'],'UNPAID',$keywords);
			
			//June
			$jun_date = $year.'-06-';
			$jun_payment = get_month_wise_fees($jun_date, $item['org_id'],'UNPAID',$keywords);
			
			//July
			$jul_date = $year.'-07-';
			$jul_payment = get_month_wise_fees($jul_date, $item['org_id'],'UNPAID',$keywords);
			
			//August
			$aug_date = $year.'-08-';
			$aug_payment = get_month_wise_fees($aug_date, $item['org_id'],'UNPAID',$keywords);
			
			//September
			$sep_date = $year.'-09-';
			$sep_payment = get_month_wise_fees($sep_date, $item['org_id'],'UNPAID',$keywords);
			
			//October
			$oct_date = $year.'-10-';
			$oct_payment = get_month_wise_fees($oct_date, $item['org_id'],'UNPAID',$keywords);
			
			//November
			$nov_date = $year.'-11-';
			$nov_payment = get_month_wise_fees($nov_date, $item['org_id'],'UNPAID',$keywords);
			
			//December
			$dec_date = $year.'-12-';
			$dec_payment = get_month_wise_fees($dec_date, $item['org_id'],'UNPAID',$keywords);
			}else{
			// January
			$jan_date = $year.'-01-';
...
<div class="share-calcuation-panel">
	<?php
		...
		if(isset($keywords) && $this->session->userdata('user_type') === 'Org Admin') {
			$allocation_total_amount = get_year_wise_total_amount($the_year, $keywords,'UNPAID','Org Admin');
		}else{
		$allocation_total_amount = get_year_wise_total_amount($the_year, $keywords, 'UNPAID');
		}
	?>
	...
	<span class="click-to-download" onclick="window.location.href='<?php echo base_url('financeapi/download?RPTYPE=UNPAID&keywords='.$keywords.'&year='.$year.'&month='.$month); ?>'">Download</span>
</div>