<?php require_once APPPATH.'modules/common/header.php' ?>
					...
					<div class="panel-body" style = "padding: 10px;">
						<div class="table-responsive">
							...
							<div class="col-lg-12" style="margin-bottom: 10px;">
								...
								<div class="col-md-3 ">
									<strong class="filter-label" style = 'color: white;background-color: #1b75bc;display: flex;justify-content: center; padding: 3%;border: 2px solid #fffdfd;border-radius: 25px;'>
										<a style = 'color: #ffffff; font-size: 16px;' href="<?php echo base_url('administrator/dashboard/doc_image'); ?>">Total Upload by Doctors</a>
									</strong>
								</div>
							...
							<div class="top-search-filter-section">
								...
								<div class="date-to-date-search" style="width:255px">
									<div class="row">
										<div style="width:100%;padding: 0 15px;">
											<strong class="inline-date-src to_label">Select Date</strong>
											<input style="width:115px;text-align:center;" type="text" placeholder="yyyy-mm-dd" class="form-control datepicker inline-date-src" id="fromDate" /> 
											<input style="width:115px;text-align:center;" type="hidden" placeholder="yyyy-mm-dd" class="form-control datepicker inline-date-src" id="toDate" />
											<input style="width:115px;text-align:center;" type="hidden" placeholder="" class=" " id="today" value = '<?php echo date('Y-m-d');?>' />
										</div>
									</div>
								</div>
								...
								<div class="search-input-bx" style="width: 150px;display: none">
								...
								<div class="search-input-bx" style="width: 230px;">
									...
									<div class="col-lg-6">
										<select name="center" class="form-control inline-src-right" id="cenTers">
											<option  value="" selected="selected">Select a Organization first</option>
							...
							<div id="postList">
							<div style = "margin-bottom: 0.5%;"><span style="font-size: 20px;display: inline-block;border: 2px solid #0a0;background: rgba(0,220,0, 0.3);border-radius: 3px;padding: 3px 10px;"><strong>Today's Count :</strong> <span id="totalPatients"><?php echo $items_count; ?></span></span></div>
	...
	<script type="text/javascript">
		function searchFilter(page_num) {
			...
			$.ajax({
				type: 'POST',
				url: '<?php echo base_url(); ?>pfilter/doc_image_today/'+page_num,
				data:'page='+page_num+'&from_date='+from_date+'&doctor='+doctor+'&centers='+cenTers+'&org='+organization,
				dataType:'json',
				beforeSend: function () {
...
function upload(doc,center)
{
	...
	$.ajax({
		type: 'POST',
		url: '<?php echo base_url(); ?>pfilter/image_patient_list_upload_today/',
		data:'doc='+doc+'&center='+center+'&from_date='+from_date+'&to_date='+to_date+'&year='+year+'&month='+month+'&today='+today,
		dataType:'json',
		beforeSend: function () {
...
function entry(doc,center)
{
	...
	$.ajax({
		type: 'POST',
		url: '<?php echo base_url(); ?>pfilter/image_patient_list_entry_today/',
		data:'doc='+doc+'&center='+center+'&from_date='+from_date+'&to_date='+to_date+'&year='+year+'&month='+month+'&today='+today,
		dataType:'json',
		beforeSend: function () {
...
<script type="text/javascript">
		$(document).ready(function(){
			...
			$(document).on('change', '#cenTers', function(){
				...
				$.ajax({
					type : "POST",
					url : baseUrl + "register/get_doctors",
					data : {center_id:center},
					dataType : "json",
					success : function (data) {
						if(data.status == "ok")
						{
							$('#doctor').html(data.content);
							$('#loader').hide();
							return false;
						}else
						{
