<?php require_once APPPATH.'modules/common/header.php' ?>
				...
				<div class="panel-wrapper collapse in put-relative" aria-expanded="true">
					<div class="panel-body">
						<div class="table-responsive">
								...
								<div class="search-input-bx" style="width: 250px;">
									...
									<div class="col-lg-7">
										<select id="doctor" class="form-control inline-src-right">
											...
											?>
											<option value="<?php echo $doctor['doctor_full_name']; ?>"><?php echo $doctor['doctor_full_name']; ?></option>
											<?php endforeach; ?>
										</select>
									</div>
								</div>
							...
							<div class="col-lg-12" style="margin: 1% 0%">
								<div class="col-md-2 ">
									<strong class="filter-label" style = 'color: #1b75bc;text-decoration: underline;'>
										<a href="<?php echo base_url('administrator/dashboard/doc_image_today'); ?>">Today Upload by Doctors</a>
									</strong>
								</div>
							...
							<div id="postList">
							<div><span style="display: inline-block;border: 2px solid #0a0;background: rgba(0,220,0, 0.3);border-radius: 3px;padding: 3px 10px;"><strong>Total Upload :</strong> <span id="totalPatients"><?php echo $items_count; ?></span></span></div>
								<table class="table">
									<thead>
										<tr>
											<th>SL.</th>
	...
	<script type="text/javascript">
		function searchFilter(page_num) {
			...
			$.ajax({
				type: 'POST',
				url: '<?php echo base_url(); ?>pfilter/doc_image/'+page_num,
				data:'page='+page_num+'&from_date='+from_date+'&to_date='+to_date+'&year='+year+'&month='+month+'&doctor='+doctor,
				dataType:'json',
				beforeSend: function () {
...
function upload(doc,center)
{
	...
	$.ajax({
		type: 'POST',
		url: '<?php echo base_url(); ?>pfilter/image_patient_list_upload/',
		data:'doc='+doc+'&center='+center+'&from_date='+from_date+'&to_date='+to_date+'&year='+year+'&month='+month,
		dataType:'json',
		beforeSend: function () {
...
function entry(doc,center)
{
	...
	$.ajax({
		type: 'POST',
		url: '<?php echo base_url(); ?>pfilter/image_patient_list_entry/',
		data:'doc='+doc+'&center='+center+'&from_date='+from_date+'&to_date='+to_date+'&year='+year+'&month='+month,
		dataType:'json',
		beforeSend: function () {
