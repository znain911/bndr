	...
	<div class="row">
		<div class="col-lg-12">
			<div class="add-href-unit">
				<strong style="margin-top:15px;" class="pull-left">DOCTORS</strong>
				<?php if ($this->session->userdata('user_type') !== 'Org Admin'):?>
				<a href="<?php echo base_url('doctors/create'); ?>" class="add-vst-button pull-right"><i class="fa fa-plus-square"></i> ADD NEW DOCTOR</a>
				<?php endif;?>
			</div>
						...
						<div class="table-responsive put-relative">
							<table id="dataTable" class="table">
								...
								<tbody>
									<?php
										$sl = 1;
										$items = $this->Doctor_model->get_all_items();
										if(count($items) !== 0):
										foreach($items as $item):
									?>
									<tr>
										<td><?php echo $sl; ?></td>
										<td><?php echo $item['doctor_full_name']; ?></td>
										<td><?php echo $item['doctor_phone']; ?></td>
										<td><?php echo $item['org_name']; ?></td>
										<td><?php echo $item['orgcenter_name']; ?></td>
										<td><?php echo date("d M, Y", strtotime($item['doctor_dateof_birth'])); ?></td>
										<td><?php echo get_age($item['doctor_dateof_birth']); ?></td>
										<td><?php echo date("d M, Y", strtotime($item['doctor_create_date'])).' '.date("g:i A", strtotime($item['doctor_create_date'])); ?></td>
										<?php if ($this->session->userdata('user_type') !== 'Org Admin'):?>
										<td>
											<?php if($item['doctor_status'] == '1'): ?>
										...
										<td class="jsgrid">
											<button onclick="window.location.href='<?php echo base_url('doctors/edit/'.$item['doctor_id']); ?>'" class="jsgrid-button jsgrid-edit-button" type="button" title="Edit"></button>
											<button data-item="<?php echo $item['doctor_id']; ?>" class="remove-btn jsgrid-button jsgrid-delete-button" type="button" title="Delete"></button>
										</td>
										<td class="text-center">
											<a data-id="<?php echo $item['doctor_id']; ?>" class="btn btn-block change-password row-action-change-password" data-target="#modal-without-animation" data-toggle="modal">Change Password</a>
										</td>
										<?php endif; ?>
									</tr>
									<?php 
									...
									?>
										<tr><td colspan="13" class="text-center">No new doctor in your Organization.</td></tr>
										<?php endif; ?>
								</tbody>
							</table>
						</div>
