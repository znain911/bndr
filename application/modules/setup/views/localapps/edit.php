<?php
	$get_info = $this->Centers_model->get_info($id);
	if($get_info != true)
	{
		redirect('not-found');
	}
	...
	<div class="row">
		<div class="col-lg-12">
			<div class="panel panel-default block2">
				<div class="panel-heading text-center">UPDATE CENTER</div>
					...
					<div class="panel-body">
							...
							<div class="col-md-4">
								<select name="organization" class="form-control" id="selectedOrganization">
									...
									<?php 
										...
										foreach($orgs as $org):
									?>
									<option value="<?php echo $org['org_id']; ?>" <?php echo ($get_info['api_app_org_id'] == $org['org_id'])? 'selected' : null; ?>><?php echo $org['org_name']; ?></option>
									<?php endforeach; ?>
								</select>
							</div>
						</div>
						<div class="form-group">
							...
							<div class="col-md-4">
								<select name="center" class="form-control" id="cenTers">
									...
									<?php 
										$centers = $this->Centers_model->get_all_centers($get_info['api_app_org_id']);
										foreach($centers as $center):
									?>
									<option value="<?php echo $center['orgcenter_id']; ?>" <?php echo ($get_info['api_app_center_id'] == $center['orgcenter_id'])? 'selected' : null; ?>><?php echo $center['orgcenter_name']; ?></option>
									<?php endforeach; ?>
								</select>
							</div>
						</div>
						<div class="form-group">
							...
							<div class="col-md-4 control-label">
								<label><input type="radio" name="status" value="YES" <?php echo ($get_info['api_app_is_active'] == 'YES')? 'checked' :null ; ?> />&nbsp;&nbsp;Active</label>&nbsp;&nbsp;&nbsp;&nbsp;<label><input type="radio" name="status" value="NO" <?php echo ($get_info['api_app_is_active'] == 'NO')? 'checked' :null ; ?> />&nbsp;&nbsp;Inactive</label>
							</div>
						</div>
					</div>
		...
		$(document).ready(function(){
			$("#createForm").validate({
				...
				submitHandler : function () {
					...
					$.ajax({
						type : "POST",
						url : baseUrl + "setup/centers/update",
