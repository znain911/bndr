...
class Administrator_model extends CI_Model {
	...
	{
		$query = $this->db->query("SELECT * FROM starter_owner 
	...
	{
		$query = $this->db->query("SELECT * FROM starter_owner WHERE starter_owner.owner_email='$email' LIMIT 1");
		return $query->row_array();
	}
	
	public function check_phone($phone)
	{
		$query = $this->db->query("SELECT * FROM starter_owner WHERE starter_owner.owner_phone='$phone' LIMIT 1");
		return $query->row_array();
	}
	...
	{
		$query = $this->db->query("SELECT * FROM starter_manages ORDER BY starter_manages.manage_id ASC");
		return $query->result_array();
	}
	...
	{
		$query = $this->db->query("SELECT org_id, org_name FROM starter_organizations ORDER BY org_id ASC");
		return $query->result_array();
	}
